  ...
  <div class="facet-list">
    ...
    <div class="facet" v-for="(facet, facetName, $index) in facets" :key="$index"
      ...
      <div v-for="(value, $index) in facet.facetValues" :key="$index" v-if="!value.selected">
        <span v-on:click.prevent="toggle(facetName, value.name)">
        ...
        <i v-if="!!negate" class="fa fa-ban facet-add-neg" v-on:click.prevent="negate(facetName, value.name)" :title="value.name"></i>
      </div>
